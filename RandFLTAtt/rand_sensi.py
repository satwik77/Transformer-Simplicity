...
out_file= 'out/sensi_uni_exp_A.csv'
out_grouped= 'out/sensi_uni_group_A.csv'
...
parser.add_argument('-depth', type=int, default=4, help='Layers')
parser.add_argument('-head', type=int, default=1, help='heads')
...
run_name = args.run + '_'+ 'h{}d{}t{}p{}'.format(args.d_model, args.depth, args.trials, args.pos_encode) + '_'+ str(np.random.choice(100000))
...
clf = TransformerCLF(ntoken =4, noutputs=1,  d_model=d_model, nhead=args.head, d_ffn=d_ffn, nlayers=args.depth, dropout=0.0, pos_encode_type =pos_encode)
...
cols = ['run_name', 'sample_size', 'len', 'd_model', 'depth', 'heads', 'pos_encode', 'is_bias', 'weight_init', 'inp_init', 'trial', 'sensi_count', 'avg_sensi']
df= pd.DataFrame(data=[], columns=cols)
fixed_data= [run_name, sample_size, inp_len, d_model, depth, args.head, pos_encode, False, weight_init, inp_init]

cols_group = ['run_name', 'sample_size', 'num_trials', 'len', 'd_model', 'depth', 'heads', 'pos_encode', 'is_bias', 'weight_init', 'inp_init', 'avg_sensi', 'std_sensi']
big_df = pd.DataFrame(data=[], columns=cols_group)
group_data= [run_name, sample_size, args.trials, inp_len, d_model, depth, args.head, pos_encode, False, weight_init, inp_init]
...
for trial in range(args.trials):
    ...
    start_time = time()
    clf.init_weights(weight_init= weight_init, inp_init=inp_init, dec_init= 1.0)  ### inp_init and dec_init are not used
